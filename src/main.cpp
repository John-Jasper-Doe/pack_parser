...
//      printf("recvfrom() error: %d\n",WSAGetLastError());
    ...
    if (buff[1] == 0x09) {++traf_count[0]; traf_size[0]+=n;}
    else if (buff[1] == 0x0a) {++traf_count[1]; traf_size[1]+=n;}
    else {++traf_count[2];traf_size[2]+=n;}
    ...
    printf("09: = %lu/%lu, 0a: = %lu/%lu, other: = %lu/%lu\n",
           ...
           traf_count[1], traf_size[1],
           traf_count[2], traf_size[2]);
  }
